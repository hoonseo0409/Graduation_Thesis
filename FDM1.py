...
def cal_box(event):   #자기 자신 전위빼고 나머지 전위 기여 계산
  ...
  for i in range(len(Pos_lst)):
      Pot_lst[Pos_lst[i][0]][Pos_lst[i][1]] += selfpot_cons
      if (Pos_lst[i][0] != 0):
          Pot_lst[Pos_lst[i][0]-1][Pos_lst[i][1]] += selfpot_cons
      if (Pos_lst[i][1] != 0):
          Pot_lst[Pos_lst[i][0]][Pos_lst[i][1]-1] += selfpot_cons
      if ((Pos_lst[i][0] != 0) and (Pos_lst[i][1] != 0)):
          Pot_lst[Pos_lst[i][0]-1][Pos_lst[i][1]-1] += selfpot_cons
  ...
  for i in range(xnumber):
      for j in range(ynumber):
        ...
        Pot_lst[i*distance][j*distance]+=add
  ...
  while (mean_diser >= diserl):
    sum_diser = 0.
    number=0.
    if ((distance >= mheight) or (distance >= mwidth)):
        number = 1.
    for y in range(1, mheight - 1):
      for x in range(1, mwidth - 1):
        if ((x % distance != 0) or (y % distance != 0)):  # distance 격자점은 건너 뜀
          Pot_lst[x][y] = (relex / 4.) * (Pot_lst[x][y - 1] + Pot_lst[x + 1][y] + Pot_lst[x][y + 1] + Pot_lst[x - 1][y])
        else:
          test = (relex / 4.) * (Pot_lst[x][y - 1] + Pot_lst[x + 1][y] + Pot_lst[x][y + 1] + Pot_lst[x - 1][y])
...
#FDM 방식으로 전위 색칠하기
  ...
  for i in range(mwidth):
      for j in range(mheight):
        c.create_oval(i, j, i, j, outline='#%02x%02x%02x' % (int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc)),
                      fill='#%02x%02x%02x' %  (int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc)), width=1)
  ...
  for y in range(1,mheight-1):
    for x in range(1,mwidth-1):
      if((x%distance!=0)or(y%distance!=0)):
        ...
        potsum = 0.
        for k in range(len(Pos_lst)):
          if(((Pos_lst[k][0]==x)and(Pos_lst[k][1]==y))or((Pos_lst[k][0]==x+1)and(Pos_lst[k][1]==y))or((Pos_lst[k][0]==x)and(Pos_lst[k][1]==y+1))or((Pos_lst[k][0]==x+1)and(Pos_lst[k][1]==y+1))):
            potsum+=selfpot_cons
  ...
  for i in range(mwidth):
      for j in range(mheight):
        ...
        c2.create_oval(i, j, i, j, outline='#%02x%02x%02x' % (int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc)),
                      fill='#%02x%02x%02x' % (int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc), int(255 * (Pot_lst[i][j] / maxpot)**coc)), width=1)
